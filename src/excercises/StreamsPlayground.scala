...
object EmptyStream extends MyStream[Nothing] {
  ...
  def take(n: Int): MyStream[Nothing] = this //first n elements out of this stream
}
...
class Cons[+A](hd: A, tl: => MyStream[A]) extends MyStream[A] {
  ...
  def filter(predicate: A => Boolean): MyStream[A] =
    ...
    else tail.filter(predicate) //preserves lazy evaluation! (only first element is evaluated)
